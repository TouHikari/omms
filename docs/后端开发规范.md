...
- **URL 路径**: 全小写，连字符分隔 (kebab-case) 或保持简洁的资源名词。
  - 正确: `/api/medical-records`, `/users/{id}/profile`
...
- 认证方式：JWT，登录成功后返回 `accessToken`，请求时在 `Authorization` 中使用 `Bearer <token>`。
- 统一依赖：
  - 获取当前用户：`get_current_user`
  - 基础鉴权：在路由聚合时为非 `auth` 路由统一添加 `Depends(require_auth)`
  - 角色校验：`Depends(require_role_in(["ADMIN", "DOCTOR"]))`
  - 权限码校验：`Depends(require_perm_codes(["record:manage"]))`
- 示例：
  ```python
  from fastapi import APIRouter, Depends
  from app.core.auth import require_auth
  from app.core.permissions import require_role_in, require_perm_codes
  ...
  @router.get(
      "/records",
      summary="病历列表查询",
      description="按筛选条件查询病历列表",
  )
  async def list_records(user = Depends(require_role_in(["ADMIN", "DOCTOR"]))):
      # 业务逻辑...
