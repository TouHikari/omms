...
- **字段命名**：
  - **注意**：为了兼容现有前端代码，接口 JSON 数据字段采用 **camelCase**（小驼峰，如 `deptId`）。
  - 后端 Pydantic 模型需配置 `alias_generator` 或 `Field(alias=...)` 将其映射为 Python 的 **snake_case**（下划线，如 `dept_id`）。
...
- GET `/api/appointments`
  - 描述：预约列表查询
  - 查询参数：`status?: 'pending'|'completed'|'cancelled'`，`date?: 'YYYY-MM-DD'`，`deptId?: number`，`doctorId?: number`，`page?: number`，`size?: number`
  - 响应：`{ items: Appointment[], total: number, page: number, size: number }`
...
- 分页：`GET /api/appointments` 默认分页，`size` 建议 20–50
